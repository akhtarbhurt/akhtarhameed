...
const { jsPDF } = window.jspdf; // Adjust this line to properly access jsPDF

const generatePDF = () => {
  // Select the resume content
  const resumeContent = document.querySelector('.resume-show');

  if (resumeContent) {
    html2canvas(resumeContent, { useCORS: true })
      .then((canvas) => {
        const imgData = canvas.toDataURL('image/png');
        const pdf = new jsPDF({
          orientation: 'p', // 'p' for portrait, 'l' for landscape
          unit: 'mm',
          format: 'a4' // You can change this to other formats
        });
